...
{	
	...
	container.removeChild(document.getElementById('trCurrency' + count_currencies));
...
function update(id)
{
	...
	var fromOptionCurrency = fromSelectCurrency.options[fromSelectCurrency.selectedIndex];
	...
	for (i = 0; i < count_currencies; i++)
	{
		...
		var toOptionCurrency = toSelectCurrency.options[toSelectCurrency.selectedIndex];
...
{
	document.getElementById('btnAdd').addEventListener('click', function(){ addCurrency(false); });
